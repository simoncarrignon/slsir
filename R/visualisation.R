...
visualize <- function(allpop,timeseries,xsize,ysize,foldername=FALSE,width=1200,height=900){
    ...
    plot(1:nrow(timeseries),timeseries[,2],col=sircol[2],type="l",lwd=2,ylim=c(0,N),xlab="time",ylab="# infected")
    lines(1:nrow(timeseries),timeseries[,3],col=sircol[3],type="l",lwd=2)
    lines(1:nrow(timeseries),timeseries[,1],col=sircol[1],type="l",lwd=2)
