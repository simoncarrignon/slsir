...
aa=lapply(unlist(lapply(list.dirs(name),list.files,pattern="outputmodel.bin",full.names=T)),function(f){load(f);return(outputmodel)})
outputmodel=do.call("rbind",aa)
#outputmodel=outputmodel[outputmodel$inf <.02 & outputmodel$sat >10,]
outputmodel=outputmodel[-which(outputmodel$max_infect <4),] #we remove the results where no outbreak happend
outputmodel$distances=(1-bdistance(outputmodel$time_max) + bdistance(outputmodel$max_infect))/2
```
...
best=outputmodel[order(outputmodel$distances),][1:1000,]
worst=outputmodel[order(outputmodel$distances,decreasing=T),][1:1000,]
...
### right panel
subresults=outputmodel[sample(nrow(outputmodel),20000),]
...
n=1000
posterior=list(
               best=outputmodel[order(outputmodel$distances),][1:n,],
               worst=outputmodel[order(outputmodel$distances,decreasing=T),][1:n,],
               time_max=outputmodel[order(outputmodel$time_max,decreasing=T),][1:n,],
               time_max150=outputmodel[order(outputmodel$time_max150,decreasing=T),][1:n,],
               time_max250=outputmodel[order(outputmodel$time_max250,decreasing=T),][1:n,],
               wtime_max150=outputmodel[order(outputmodel$time_max150,decreasing=F),][1:n,],
               wtime_max250=outputmodel[order(outputmodel$time_max250,decreasing=F),][1:n,],
               max_infect=outputmodel[order(outputmodel$max_infect,decreasing=F),][1:n,],
               max_infect150=outputmodel[order(outputmodel$max_infect150,decreasing=F),][1:n,],
               max_infect250=outputmodel[order(outputmodel$max_infect250,decreasing=F),][1:n,],
               wmax_infect150=outputmodel[order(outputmodel$max_infect150,decreasing=T),][1:n,],
               wmax_infect250=outputmodel[order(outputmodel$max_infect250,decreasing=T),][1:n,]
               )
...
```{r,eval=T,out.width="33%",results="asis",fig.show="hold",fig.cap=" All posterior distributions for the 6 parameters used in our simulations. The white area represents the distribution of the parameters of all simulations (the priors), the blue area represents the distribution for the top 1000 simulations rank using δ while the shaded area represent the parameters of the 1000 simulations with the lowest number of infected people after 150 timesteps.Left column represents the parameters value to switch from Non-Adherent to Adherent, middle column from Adherent to Non-Adherent, and right column the paramaters that defines the probability and the radius of social learning. " }
    ...
    plot2dens(prior=(outputmodel$inf),A=(posterior$best$inf),B=(posterior$max_infect150$inf),cols=colors,from=0,to=1,main="posterior inflection point",xlab=expression(nu),xaxt="n")
    plot2dens(prior=(outputmodel$inf_r),A=(posterior$best$inf_r),B=(posterior$max_infect150$inf_r),cols=colors,from=0,to=1,main="posterior revert inflection point",xlab=expression(nu[r]),xaxt="n")
    legend("topleft",legend=c("prior","final time","150 timesteps"),fill=c(0,colorbest,colorbest),density=c(NA,NA,20))
    plot2dens(prior=1-outputmodel$pind,A=1-posterior$best$pind,B=1-posterior$max_infect250$pind,cols=colors,from=0,to=1,main="posterior proba social learning",xlab="proba social learning")


    plot2dens(prior=log10(outputmodel$sat),A=log10(posterior$best$sat),B=log10(posterior$max_infect150$sat),cols=colors,from=-1,to=3,xlab=expression(kappa),main="",xaxt="n",log=T)
    plot2dens(prior=log10(outputmodel$sat_r),A=log10(posterior$best$sat_r),B=log10(posterior$time_max150$sat_r),cols=colors,from=-1,to=3,main="",xlab=expression(kappa[r]),log=T)

    plot2dens(prior=outputmodel$sl_rad,A=posterior$best$sl_rad,B=posterior$max_infect150$sl_rad,cols=colors,from=1,to=100,main="",xlab="radius social learning")
...
allexpe=lapply(modelnames,function(name){
               aa=lapply(unlist(lapply(list.dirs(name),list.files,pattern="outputmodel.bin",full.names=T)),function(f){load(f);return(outputmodel)})
               outputmodel=do.call("rbind",aa)
               if(sum(which(outputmodel$max_infect <4)>2))outputmodel=outputmodel[-which(outputmodel$max_infect <4),]
               outputmodel$distances=(1-bdistance(outputmodel$time_max) + bdistance(outputmodel$max_infect))/2
               outputmodel$distances2=(1-bdistance(outputmodel$time_max150) + bdistance(outputmodel$max_infect150))/2
               outputmodel$distances3=(1-bdistance(outputmodel$time_max250) + bdistance(outputmodel$max_infect250))/2
               return(outputmodel)})
...
for(d in nvl){
    best=outputmodel[rank(outputmodel$distances)<d,]
    par(mar=c(4,4,1,1))
...
Distribution of the different metrics used to approximate the posteriors for all 4 variantes:
...
lapply(names(allexpe),function(mname){
       outputmodel=allexpe[[mname]]
       plot(outputmodel$time_max,outputmodel$max_infect,col=adjustcolor(color.gradient(outputmodel$distances,c(mygreen,"yellow", myred)),.2),main=mname,pch=20,xlab="Time Max",ylab="Max Infected")
       sset=sort(outputmodel$distances)
