...
for(n in names(listallposterior)){
    ...
    for(mod in names(sbest)){
        ...
        tryCatch({hdr.boxplot.2d(best$inf_r,log10(best$sat_r),prob=seq(20,100,10),shadecols=adjustcolor(modelcol[mod],1),xlim=c(0,1),ylim=c(-1,3),xlab="revert inf. point",ylab="revert steepness (log10)")},error=function(e){print(mod)})
    }
    ...
    for(mod in names(sbest)){
        ...
        if(mod=="burnin100BestSLS")plot.new()
        tryCatch({hdr.boxplot.2d(best$inf,log10(best$sat),prob=seq(20,100,10),shadecols=adjustcolor(modelcol[mod],.9),xlim=c(0,1),ylim=c(-1,3),xlab="inflexion point",ylab="steepness (log10)")},error=function(e){print(mod)})
    }
